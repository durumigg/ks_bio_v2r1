...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        title = title or _("rotcalibration")
        ...
        self.distances = ['100', '300', '500', '750'] # 0.1g == 38.5mm  100mg  == 38.5
        ...
        self.buttons = {
            ...
            'rotmacro': self._gtk.Button("settings", _("Extrude calibration"), "color2"),
            'resetextrude': self._gtk.Button("settings", _("Extrude data reset"), "color2"),
        ...
        self.buttons['rotmacro'].connect("clicked", self.menu_item_clicked, {
            "panel": "rotmacro"
        })
        self.buttons['resetextrude'].connect("clicked", self.reset_data_extrude)
        ...
        self.labels['extrude_dist'] = Gtk.Label(_("Distance (mg)"))
        ...
        grid.attach(xbox, 0, 0, 6, 1)
        self.labels["extrude_data"] = Gtk.Label(_("extrude: "))
        if self._screen.vertical_mode:
            ...
            grid.attach(self.labels["extrude_data"], 0, 3, 2, 1)
            grid.attach(self.buttons['rotmacro'], 2, 2, 2, 1)
            grid.attach(self.buttons["resetextrude"], 4, 2, 2, 1)
            #grid.attach(self.buttons['load'], 1, 2, 1, 1)
            #grid.attach(self.buttons['unload'], 2, 2, 1, 1)
            #grid.attach(self.buttons['retract'], 3, 2, 1, 1)
            grid.attach(distbox, 0, 4, 3, 1)
            grid.attach(speedbox, 3, 4, 3, 1)
    ...
    def reset_data_extrude(self, widget):
        self.labels[f"extrude_data"].set_text(f"extrude: ")
        self.cal_dist = 0
    ...
    def extrude(self, widget, direction):
        ...
        temp_dist = temp_dist * 0.385
        self._screen._ws.klippy.gcode_script(KlippyGcodes.EXTRUDE_REL)
        self._screen._send_action(widget, "printer.gcode.script",
                                  {"script": f"G1 E{direction}{temp_dist} F{self.speed * 60}"})
        #{"script": f"G1 E{direction}{self.distance} F{self.speed * 60}"})
        self.cal_dist = self.cal_dist + self.distance
        self.labels[f"extrude_data"].set_text(f"extrude: {self.cal_dist:.2f}")
