...
class Panel(ScreenPanel):
    ...
    def load_gcode_macros(self):
        ...
        for macro in self._printer.get_gcode_macros():
            if macro == "CHANGE_ROTATION_DISTANCE":
                self.options[macro] = {
                    "name": macro,
                    "section": f"displayed_macros {self._screen.connected_printer}",
                    "type": "binary"
                }
                logging.debug(f"macromacromacromacro {self.options[macro]}")
        ...
        for macro in list(self.options):
            if "type" not in self.options[macro]:  # 안전 검사 추가
                logging.error(f"Macro missing 'type': {macro}, data: {self.options[macro]}")
                continue
            self.add_option('options', self.options, macro, self.options[macro])

        # 매크로 정렬 및 GUI 업데이트
        macros = sorted(self.macros, reverse=False, key=str.casefold)
